...
const displaySingleData = (allNews) => {
  ...
  allNews.forEach((news) => {
    ...
    <div class="flex justify-between align-items-center">
      ...
      <div>
        <button id="show-details" onclick="showModalDetails(${news.category_id})" class=" type="button" data-modal-toggle="defaultModal" "><i class="fa-solid fa-arrow-right"></i></button>
      </div>
      </div>
      </div>
      </div>
    ...
    // const length = allNews.length;
     newsLength.innerText = `${allNews.length} items found for this category.`;
    console.log(length);
    // if(length > 0){
    //   newsLength.innerText = `${length} items found for category Entertainment`
    // }
    // else{
    //   newsLength.innerText = "No items found!!"
    // }
  });
  toggleSpinner(false);
...
const showModalDetails = (all_id) => {
  console.log(all_id);
  const modalContainer = document.getElementById('defaultModal');
  all_id.forEach(id => {
